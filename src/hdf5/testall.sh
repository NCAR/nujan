...
badparms() {
  ...
  echo "./testall.sh v1  chunked 0 string14 2"
  echo "./testall.sh v2  contig  0 vstring  all"
...
testOne() {
  ...
  if [ "$compress" != "0" -a "$dtype" == "vstring" ]; then
    ...
    cmd="java -cp tdcls hdfnetTest.Testa \
      -bugs 10 \
      -dtype $dtype \
      -dims $dims \
    ...
    configMsg="./testall.sh v$fileVersion $chunk $compress $dtype $rank"
